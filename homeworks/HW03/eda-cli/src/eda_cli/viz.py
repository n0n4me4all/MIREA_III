...
def plot_boxplots(df: pd.DataFrame, out_path: PathLike, max_columns: int = 6) -> Path:
    ...
    if df.empty or not cols:
        ...
        data = [numeric_df[c].dropna().values for c in cols if not numeric_df[c].dropna().empty]
        labels = [c for c in cols if not numeric_df[c].dropna().empty]
        ...
        if not data:
            ...
            fig, ax = plt.subplots(figsize=(8, 4 + len(labels) * 0.4))
