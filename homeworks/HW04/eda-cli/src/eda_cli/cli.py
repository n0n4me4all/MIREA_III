...
def report(
    ...
    top_k: int = typer.Option(5, help="Сколько top-значений выводить для категориальных признаков."),
    ...
    top_cats = top_categories(df=df,top_k=top_k)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть категориальные признаки с высоким числом уникальных значений: **{quality_flags['has_high_num_unique']}**\n")
        f.write(f"- Дубли в ID-полях: **{quality_flags['has_id_duplicates']}**\n")
        if quality_flags['has_id_duplicates']:
            f.write(f"  - {quality_flags['sus_id_columns']}\n")
        f.write(f"- Много нулей в числовых колонках: **{quality_flags['has_many_zero_values']}**\n\n")
        ...
        if not top_cats:
            ...
            f.write(f"Top-{top_k} значений категориальных колонок сохранены в `top_categories/`\n\n")

        f.write("## Гистограммы числовых колонок\n\n")
        f.write(f"Гистограммы для максимум {max_hist_columns} числовых колонок сохранены как `hist_*.png`\n\n")

        f.write("## Boxplot числовых признаков\n\n")
        f.write(f"График boxplot сохранён в `boxplots.png`.\n\n")
    ...
    plot_boxplots(df, out_root / "boxplots.png")

    typer.echo(f"Отчёт сгенерирован в каталоге: {out_root}")
    typer.echo(f"- Основной markdown: {md_path}")
    typer.echo("- Табличные файлы: summary.csv, missing.csv, correlation.csv, top_categories/*.csv")
    typer.echo("- Графики: hist_*.png, missing_matrix.png, correlation_heatmap.png, boxplots.png")
