...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool] = Field(
        ...,
        description="Полный набор флагов качества данных",
    )
...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор флагов качества по CSV-файлу с использованием расширенных эвристик",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    и возвращает флаги оценки качества данных. Такие, как:
    - too_few_rows, too_many_columns, too_many_missing
    - has_constant_columns (новое)
    - has_high_num_unique (новое) 
    - has_id_duplicates (новое) 
    - has_many_zero_values (новое) 
    """
    ...
    print(
        ...
        f"n_flags={len(flags_bool)} latency_ms={latency_ms:.1f} ms"
    )
