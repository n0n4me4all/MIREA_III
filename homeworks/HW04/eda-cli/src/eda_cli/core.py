...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    - колонки, с одинаковыми значениями;
    - категориальные признаки с высоким числом уникальных значений;
    - дубли в ID-полях;
    - высокая доля нулей;
    """
    flags: Dict[str, Any] = {}
    ...
    high_card_cols = [c for c in cat_cols if df[c].nunique(dropna=True) > 0]
    flags["has_high_num_unique"] = len(high_card_cols) > 0

    id_columns = [c for c in ["id", "user_id"] if c in df.columns]
    id_cols = [c for c in id_columns if df[c].nunique(dropna=False) < n_rows]
    flags["has_id_duplicates"] = len(id_cols) > 0
    flags["sus_id_columns"] = id_cols

    num_cols = df.select_dtypes(include="number").columns
    zero_cols = [c for c in num_cols if (df[c] == 0).sum() / max(n_rows, 1) >= 0.5]
    ...
    if flags["has_constant_columns"]:
        score -= 0.05
    if flags["has_high_num_unique"]:
        score -= 0.05
    if flags["has_id_duplicates"]:
        score -= 0.1
